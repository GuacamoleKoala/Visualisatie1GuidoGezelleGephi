...
function nodeActive(a) {
    ...
    if (f.attributes) {
        ...
        e = [];
        // MODIFIED: Custom list of attributes to display as per user request
        ...
        for (var i = 0; i < displayAttributes.length; i++) {
            ...
            var attrValue = f.attributes[attrKey];
            
            if (attrValue) {
                var h = '';
                if (attrConfig.isLink) {
                    h = '<span><strong>' + attrConfig.label + ':</strong> <a href="' + attrValue + '" target="_blank">' + attrValue + '</a></span><br/>';
                } else {
                    h = '<span><strong>' + attrConfig.label + ':</strong> ' + attrValue + '</span><br/>';
                }
                e.push(h);
            }
        }
        
        // OLD CODE REMOVED: (for (var attr in f.attributes) { ... })

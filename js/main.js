    ...
    dataReady = function() {//This is called as soon as data is loaded
		...
		a.iterNodes(
			function (b) { //This is where we populate the array used for the group select box
                ...
                if (groupName) {
                    // Zoek de juiste waarde van de attributen array
                    if (b.attr.attributes && Array.isArray(b.attr.attributes)) {
                        for (var i = 0; i < b.attr.attributes.length; i++) {
                            if (b.attr.attributes[i].attr === groupAttrKey) {
                                groupName = b.attr.attributes[i].val;
                                break;
                            }
                        }
                    }
...
/**
 * AANGEPASTE VERSIE VAN showCluster(a) - GEBRUIKT clusterNames
 */
